...
import static net.lafkon.petter.Petter.absScale;
import static net.lafkon.petter.Petter.customFill;
import static net.lafkon.petter.Petter.customStroke;
import static net.lafkon.petter.Petter.customStrokeWeight;
import static net.lafkon.petter.Petter.exportCurrentFrame;
import static net.lafkon.petter.Petter.globalStyle;
import static net.lafkon.petter.Petter.pdf;
import static net.lafkon.petter.Petter.relsca;
import static net.lafkon.petter.Petter.s;
...
class TileSVG extends PShapeSVG implements Tile {
			...
			if (customStroke) {
				...
				if (!strokeMode) { // setRelativeStrokeWeight
					if (tileEditor == null || !tileEditor.getG().equals(g)) { // dirty! if not tileeditor drawing
						...
						if (relsw != 0f) {
							relsw = PApplet.abs(customStrokeWeight * (1 / relsw));
						}
	...
	private void handleLFKN_VDB() {
		if (PApplet.split(filepath, ".lafkon.net").length > 1) {
			filepath = PApplet.split(filepath, ".pdf")[0] + ".svg";
		}
		try {
...
class TileShape extends PShape implements Tile {
